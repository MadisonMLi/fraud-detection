"""
Fraud Detection — Cost-Sensitive Weighted Model
================================================
Improvement over fraud_analysis.py:

1. Amount-weighted loss function
   - Every fraud transaction is weighted base_w × (1 + amount/mean_amount)
   - Guarantees every fraud weight >= base_w (8,806) >> legit weight of 1
   - High-value fraud gets proportionally MORE weight on top of the floor
   - No fraud case can ever be weighted below a legitimate transaction

2. Business cost threshold optimisation
   - Replaces F1 maximisation with a real cost function:
       Total Cost = missed fraud $ value + (false alarms × cost per alert)
   - Finds the threshold that minimises total operational cost
   - Tested across 3 cost-per-alert scenarios: $50, $200, $500

3. Outputs
   - test_fraud_external_predicted.parquet with is_fraud_predicted column
   - plots/12_cost_optimisation.png — full cost analysis visualisation
"""
...
OUT      = BASE / "test_fraud_external_predicted.parquet"
PLOT_DIR = BASE / "plots"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 3. CORRECTED AMOUNT-WEIGHTED SAMPLE WEIGHTS
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*60)
...
# ── Plot A: Weight formula comparison — old vs corrected ──────────────────────
...
ax3.set_title(
    f'Business Cost Curve — Finding the Balanced Point\n'
...
plt.suptitle('Cost-Sensitive Fraud Detection — Corrected Amount Weighting',
             fontsize=14, fontweight='bold', y=1.01)
plt.savefig(PLOT_DIR / '12_cost_optimisation.png', bbox_inches='tight')
plt.close()
print("  Saved: 12_cost_optimisation.png")
...
print(f"Plot saved to     : {PLOT_DIR}/12_cost_optimisation.png")
print("\n" + "="*60)
print("DONE")
print("="*60)
